...
CREATE TABLE IF NOT EXISTS Archetype (
  archetype_id INTEGER PRIMARY KEY AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS Pokemon (
  ...
  national_pokedex_id INTEGER NOT NULL CHECK (national_pokedex_id > 0),
  name VARCHAR(40) DEFAULT 'Unknown',
  ability VARCHAR(40),
...
--
-- update query 1
WITH pkmNoTrainer AS (
  SELECT p.u_id
  FROM Pokemon AS p
  LEFT JOIN Trainer_pkmn AS tpkm ON tpkm.u_id = p.u_id
  WHERE (p.ability LIKE 'Torrent' OR p.ability LIKE 'Water Absorb')
    AND tpkm.trainer_id IS NULL
    AND p.speed >= 8
),
...
INSERT INTO Trainer_pkmn (trainer_id, u_id)
SELECT misty.trainer_id, pkm.u_id
FROM mistyID misty, pkmNoTrainer pkm;

-- update query 2
UPDATE Item
SET cost = cost + 1000
WHERE (
  SELECT i.name, i.cost 
  FROM Item i
  JOIN Trainer_Item ti ON ti.item_id = i.item_id
  JOIN Trainer t ON t.trainer_id = ti.trainer_id
  WHERE t.name = 'Ash'
);
...
WHERE (
  SELECT speed
  FROM Pokemon p 
  JOIN Trainer_pkmn tpkm ON tpkm.u_id = p.u_id
  JOIN Trainer t ON t.trainer_id = tpkm.trainer_id
  JOIN Trainer_Archetype ta ON ta.trainer_id = t.trainer_id
  JOIN Archetype a ON a.archetype_id = ta.archetype_id
  WHERE a.archetype_name = 'Gym Leader'
);